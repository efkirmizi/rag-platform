...
| ADR-3 | Embedding modeli seçimi: **bge-m3** (reranker: **duruma göre**, aşağıya bak) | ✅ Karar (2026-07-19) · **gerçek metinle doğrulandı (2026-07-20)** | **Gerçek Türkçe korpus (27 doküman) tekrarı:** bge-m3 rerank'siz MRR 0.974 / hit@1 0.949 · Qwen3-0.6B 0.932 / 0.872 — sentetikteki fark aynı yönde ve büyüklükte. Token verimi de tekrarlandı: 1.79 vs 2.59 tok/kelime (Qwen3 ~%45 fazla). Latency: 107ms vs 180ms. **Yeni bulgu:** reranker'ın maliyet/fayda dengesi gerçek (uzun) metinde çok daha kötü — p95 107→678ms (~6×) karşılığında yalnız +0.013 MRR / +0.025 hit@1. Sentetikte +0.032 MRR'a karşılık +33ms idi. Yani reranker'ı **koşulsuz açmak artık önerilmiyor**; gecikmeye duyarlı kullanımda kapalı bırakılabilir. | Qwen3-Embedding-0.6B vs bge-m3, golden_v2 (40-sayfa confusable, %100 TR) + GPU ile ölçüldü. bge-m3 rerank'siz kalite (MRR 0.937 vs 0.896), Türkçe token verimi (1.76 vs 2.62 tok/kelime → ~%49 daha az bağlam/maliyet) ve latency'de üstün; reranker ile ikisi de tavana ulaşıyor. Not: 0.6B Qwen3 karşılaştırıldı; daha büyük Qwen3 (4B/8B) gerçek pilotta yeniden değerlendirilebilir. Rapor: `eval/results/g2-report.md`. |
...
| Konu | Sonuç | Rapor |
|---|---|---|
| Embedding + reranker (ADR-3) | bge-m3 seçildi; **gerçek metinle doğrulandı**. Reranker faydası gerçek metinde küçük, maliyeti büyük (~6× latency) | `eval/results/g2-report.md` |
| Chunking (800/150) | Küçük chunk + overlap kazandı: +0.026 MRR, +0.052 hit@1 | `eval/results/chunking-matrix.json` |
